...
import { faCode, faLink, faBars, faXmark } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-[1001] transition-all duration-500 px-4 md:px-10 pt-6">
        <div className={`mx-auto max-w-7xl flex justify-between items-center px-6 py-3 rounded-2xl transition-all duration-500 ${
          scroll || isOpen
            ? isDark
          ...
          >
            <div className={`relative p-2 rounded-lg border transition-colors duration-500 ${isDark ? "bg-[#121212] border-white/10" : "bg-white/10 border-white/20"}`}>
              <FontAwesomeIcon icon={faCode} className={`text-xl transition-colors ${isDark ? "text-gray-400" : "text-pink-500"}`} />
              {/* Pulse ring on logo */}
              <span className={`absolute inset-0 rounded-lg border opacity-0 group-hover:opacity-100 group-hover:scale-125 transition-all duration-500 ${isDark ? "border-gray-600" : "border-pink-500/50"}`} />
            </div>
            <h2 className={`text-xl font-black tracking-tighter uppercase italic transition-colors ${isDark ? "text-[#e0e0e0]" : "text-white"}`}>
              Avi Dev
            </h2>
          </motion.div>
                ...
                <a
                  ...
                  className={`text-xs font-bold tracking-[0.2em] uppercase transition-all duration-300 relative group ${
                    isActive
                      ...
                      : isDark ? "text-gray-500 hover:text-white" : "text-white/50 hover:text-white"
                  }`}
                >
                  {link.name}
                  {/* Animated underline */}
                  ...
                  {/* Hover dot */}
                  {!isActive && (
                    <span className={`absolute -bottom-1 left-1/2 w-0 h-[2px] transition-all duration-300 group-hover:w-full group-hover:left-0 rounded-full ${isDark ? "bg-gray-400" : "bg-white/40"}`} />
                  )}
                </a>
              );
            })}
          ...
          <div className="flex items-center gap-5 relative z-[1002]">
            <a href="https://drive.google.com/file/d/1MXVXgnLh9UW3OEVKifR_x-3C0VHb92rD/view?usp=drivesdk" target="_blank" className="hidden sm:block">
              <motion.button
              ...
              >
                <span className={`relative px-5 py-2 transition-all ease-in duration-75 rounded-full group-hover:bg-opacity-0
            ...
            <button
              ...
              className={`md:hidden w-10 h-10 flex items-center justify-center rounded-xl border active:scale-90 transition-all
                ${isDark ? "bg-[#1e1e1e] border-white/10 text-white" : "bg-white/10 border-white/20 text-white"}`}
            >
        ...
        {isOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setIsOpen(false)} className="fixed inset-0 z-[998] bg-black/70 backdrop-blur-md md:hidden" />
            <motion.div
            ...
            >
              <div className="mb-10">
                <span className={`font-mono text-sm tracking-widest uppercase ${isDark ? "text-gray-500" : "text-pink-500"}`}>Navigation</span>
              </div>

              <nav className="flex flex-col gap-6">
                {navLinks.map((link, i) => (
                  <motion.div key={link.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <a href={link.href} onClick={() => setIsOpen(false)}
                      ...
                      <span className={`text-xs font-mono opacity-50 ${isDark ? "text-gray-500" : "text-pink-500"}`}>0{i + 1}</span>
                      {link.name}
                    </a>
                  </motion.div>
                ))}
