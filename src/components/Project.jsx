...
const Project = () => {
  ...
  useEffect(() => {
    if (hasInteracted.current && !showAll && sectionTopRef.current) {
      const id = setTimeout(() => sectionTopRef.current.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(cards,
        ...
        { opacity: 1, y: 0, scale: 1, duration: 0.65, ease: "power3.out", stagger: 0.1,
  ...
  const handleToggle = () => { hasInteracted.current = true; setShowAll(!showAll); };

  return (
    <div ref={sectionTopRef} className="scroll-mt-24 pt-6 md:pt-20 px-6 md:px-28 py-20 bg-transparent" id="project">
      ...
      <div className="justify-center items-center flex flex-col space-y-4 mb-16">
        ...
        <motion.div
          initial={{ width: 0 }} whileInView={{ width: "100px" }} viewport={{ once: true }}
          className={`h-1.5 rounded-full ${isDark ? "bg-gray-600" : "bg-gradient-to-r from-pink-500 to-indigo-500"}`}
        />
      </div>

      {/* Grid */}
      <ul ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 overflow-visible">
              ...
              >
                ...
                >
                  ...
                  <div className="relative overflow-hidden h-[240px] m-3 rounded-[1.8rem]">
                    <a href={item.Deploy} target="_blank" rel="noreferrer" className="block h-full relative z-10">
                      ...
                      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center z-20 rounded-[1.6rem]">
                        <span className={`flex items-center gap-2 px-5 py-2 rounded-full text-white font-bold text-sm border border-white/30 backdrop-blur-md
                      ...
                      <div className={`absolute inset-0 opacity-80 pointer-events-none ${isDark ? "bg-gradient-to-t from-[#121212]" : "bg-gradient-to-t from-slate-900/60"}`} />
                    </a>

                    {/* Tech pills */}
                    <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-1.5 z-20 pointer-events-none">
                      {item.tech.map((skill, i) => (
                        <span key={i} className={`px-3 py-1 text-[10px] font-bold uppercase tracking-tighter backdrop-blur-xl border rounded-full transition-all duration-500
                  ...
                  <div className="p-7 pt-2 flex flex-col grow">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className={`text-2xl font-black tracking-tighter transition-all duration-300
                        ${isDark ? "text-white group-hover:text-gray-400" : "text-white group-hover:text-pink-400"}`}>
                        {item.title}
                      </h3>
                      <span className={`text-[10px] font-mono mt-2 uppercase tracking-widest ${isDark ? "text-gray-500" : "text-white/40"}`}>
                        #{item.id}
                      </span>
                    </div>
                    <p className={`text-sm leading-relaxed mb-8 grow line-clamp-2 font-medium ${isDark ? "text-gray-400" : "text-white/70"}`}>
                      {item.desc}
                    </p>
                    ...
                    <div className="flex items-center gap-3 mt-auto">
                      ...
                      <motion.a
                        href={item.Deploy} target="_blank" rel="noreferrer"
                        whileHover={{ scale: 1.06, boxShadow: isDark ? "0 0 20px rgba(156,163,175,0.3)" : "0 0 24px rgba(219,39,119,0.45)" }}
                        whileTap={{ scale: 0.97 }}
      ...
      <div className="mt-16 flex justify-center">
        <motion.button
          ...
          whileHover={{ scale: 1.05, boxShadow: isDark ? "0 0 24px rgba(156,163,175,0.2)" : "0 0 28px rgba(219,39,119,0.3)" }}
          whileTap={{ scale: 0.96 }}
          className="relative group inline-flex items-center justify-center px-10 py-3 font-bold"
        >
          <span className={`absolute inset-0 w-full h-full rounded-full border-2 transition-all
